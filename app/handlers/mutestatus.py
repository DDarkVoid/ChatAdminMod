...
async def handle_mutestatus(message: Message) -> None:
    ...
    mute_until = database.get_mute(connection_id, chat_id)

    if not database.is_muted(connection_id, chat_id):
    ...
    if remaining <= 0:
        database.remove_mute(connection_id, chat_id)
    ...
    await message.answer(f"🔇 Чат замьючен.\n\n⏱ Осталось: {time_text}")