...
def commands_keyboard(language: str):
    builder = InlineKeyboardBuilder()

    if language == "en":
        builder.button(
            text=".mute",
            callback_data="command_mute"
        )

        builder.button(
            text=".mutestatus",
            callback_data="command_mutestatus"
        )

        builder.button(
            text=".antispam",
            callback_data="command_antispam"
        )
...
def language_settings_keyboard(language: str):
    builder = InlineKeyboardBuilder()
...
async def send_start(
    ...
    if language == "en":
        text = (
            ...
            "<code>@ChatAdminModbot</code>"
            "</blockquote>\n\n"
            "If «Chat Automation» is missing, "
...
@router.callback_query(
    lambda callback: callback.data == "settings_language"
)
async def settings_language(
    callback: CallbackQuery
):
    language = database.get_language(
        callback.from_user.id
    ) or "ru"
    ...
    await callback.message.edit_text(
        text,
        parse_mode="HTML",
        reply_markup=language_settings_keyboard(language)
    )
...
@router.callback_query(
    lambda callback: callback.data == "commands"
)
async def commands(
    callback: CallbackQuery
):
    ...
    await callback.message.edit_text(
        text,
        parse_mode="HTML",
        reply_markup=commands_keyboard(language)
    )
...
@router.callback_query(
    lambda callback: callback.data == "command_mute"
)
async def command_mute(
    ...
    if language == "en":
        text = (
            ...
            "New messages from the muted contact "
        ...
        text = (
            ...
            "Новые сообщения собеседника будут "
...
@router.callback_query(
    lambda callback: callback.data == "command_mutestatus"
)
async def command_mutestatus(
    callback: CallbackQuery
):
    language = database.get_language(
        callback.from_user.id
...
@router.callback_query(
    lambda callback: callback.data == "command_antispam"
)
async def command_antispam(
    ...
    if language == "en":
        text = (
            "<b>.antispam</b>\n\n"
            "Automatically detects rapid message flooding "
            "from a contact.\n\n"
        ...
        text = (
            "<b>.antispam</b>\n\n"
            "Автоматически обнаруживает слишком быструю "
            "отправку сообщений собеседником.\n\n"
            "<b>Текущие настройки:</b>\n\n"
            "10 сообщений за 5 секунд.\n"
            "Собеседник получает мут на 30 секунд.\n\n"
            "<b>Использование:</b>\n\n"
            "<code>.antispam</code> — "
            "включить или выключить антиспам."
        )
...
@router.callback_query(
    lambda callback: callback.data == "back_start"
)
